...
// push 超过初始容量自动 grow：数据完整、LIFO 顺序保持、容量翻倍
static bool test_grow_overflow()
{
    QueueWorker<int> wk(2); // 极小容量，push 第 3 个即触发 grow

    // 连续 push 超过容量（自动扩容，恒成功）
    for (int i = 1; i <= 100; ++i) {
        CHECK(wk.push(i));
    }
    CHECK(wk.capacity() >= 128); // 2 → 4 → 8 → ... → 128（多次 grow）
...
// grow 后继续 push/pop 环绕复用新容量：数据完整
static bool test_grow_wrap_around()
{
    QueueWorker<long> wk(4);
    // 多次 push/pop 交织，触发 grow 且环绕
    for (int round = 0; round < 200; ++round) {
        ...
        for (long i = 0; i < 10; ++i) {
            long x = 0;
            CHECK(wk.pop(x)); // LIFO: 9..0，不校验值，只保证可弹出
        }
    }
    CHECK(wk.empty());
    return true;
...
// 多 deque × 多 thief：每次 push 都触发 grow（极小容量），无丢无重（和校验）
static bool test_multithread_grow_steal()
{
    ...
    for (int i = 0; i < OWNERS; ++i) {
        workers.push_back(std::make_unique<QueueWorker<long>>(64)); // 适中容量 → 触发多次 grow 且频率现实
    }
    ...
    for (int t = 0; t < OWNERS; ++t) {
        threads.emplace_back([&, t]() {
            ...
            for (int i = 0; i < PER; ++i) {
                ...
                workers[t]->push(v); // grow 后恒成功，无需重试
            }
        });
    }
...
int main()
{
    ...
    std::cout << "[grow] push over capacity auto-grows, data intact\n";
    test_grow_overflow();
    std::cout << "  PASS\n";

    std::cout << "[grow] push/pop interleave with wrap-around\n";
    test_grow_wrap_around();
    std::cout << "  PASS\n";
    ...
    std::cout << "[grow+MPMC] " << 4 << " owners x " << 4 << " thieves, frequent grow\n";
    test_multithread_grow_steal();
    std::cout << "  PASS\n";
