...
> 日期：2026-08-12（Day30 初测）→ 2026-08-22（Day31 重复采样复测）
...
**Day30 初测**：Before（临时移除 `ThreadPool.h` 中 3 个原子计数器的 `alignas(...)`）vs After（恢复 `alignas`），各跑 3 次取中位数。
**Day31 复测**：Part 2 接入 `benchmark_util.h` 的 `sample()`（warmup=1 + repeats=3，报告 avg/min/max/median），消除单次运行抖动。

## Part 1 — Micro：false sharing 现象（与 ThreadPool 无关，纯计数器）

| threads | adjacent | padded | speedup |
|---|---|---|---|
| 2 | ~1255 ms | ~191 ms | **6.6x** |
| 4 | ~3213 ms | ~200 ms | **16.1x** |
| 8 | ~6560 ms | ~254 ms | **25.8x** |

> 同一份代码，只差 `alignas(64)`，8 线程差 ~26 倍 —— false sharing 现象的教科书级复现。
> Before/After 两组此数据一致（该部分与 ThreadPool 无耦合，不随 alignas 变化）。

## Part 2 — Macro：ThreadPool 500k 任务

### Day30 初测（单次运行，3 次取中位数）
...
### Day31 复测（`sample()` 重复采样，warmup=1 + repeats=3）

```
--- Part 2: ThreadPool throughput (as built) ---
threadpool threads=1: avg=966.896ms min=849.189ms max=1077.13ms median=974.371ms
threadpool threads=2: avg=872.962ms min=749.628ms max=987.939ms median=881.318ms
threadpool threads=4: avg=1431.48ms min=1107.8ms max=1754.88ms median=1431.76ms
threadpool threads=8: avg=7481.46ms min=7196.25ms max=8000.1ms median=7248.03ms
```

相邻两次运行 t8 median：`7266ms / 7352ms`（波动 <2%）。

> 注意：**跨会话绝对值不可比**。同一机器热/负载变化使 t4 等绝对值在会话间漂移（如 Day30 的 ~840ms → Day31 的 ~1330ms）。因此跨会话只比较**趋势与结论**，会话内才用重复采样做定量对比。

## 结论（与预期不符）

**Micro 现象成功复现（~26x），但 ThreadPool 上 alignas 几乎无收益（±5% 噪声内）。**
...
2. **Part 2 的 threads=8 绝对性能极差**（~7.2s vs t1 的 ~0.97s），高线程下 ThreadPool 的可扩展性被锁扼杀。
